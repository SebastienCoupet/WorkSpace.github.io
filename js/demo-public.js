...
if (subEl) subEl.textContent = `${TODAY_LABEL} · Bonjour Sophie 👋`;

// ─── HELPERS ──────────────────────────────────────────────────────────────────
function getUserName() { return 'Sophie C.'; }
...
function cycleSite() {
  ...
  document.getElementById('tb-sub').textContent = `${TODAY_LABEL} · Bonjour Sophie 👋`;
...
function confirmBooking() {
  ...
  closeModal();
  showToast('Réservation confirmée ! (mode démo)');
}
...
function reserverBureau() {
  ...
  showToast('Bureau ' + selectedDesk + ' réservé ! (mode démo)');
}
...
function passerCommande() {
  showToast('Commande envoyée à Delizia · Livraison prévue dans 30 min ✓ (mode démo)');
